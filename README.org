...
def set_bearings(font, bearings, **kwargs):
  ...
  for g in font:
    ...
    if bearing_dict[key][1] != '':
      font[g].left_side_bearing = bearing_dict[key][1] * (params['scale'] or 1)
    else:
      font[g].left_side_bearing = bearing_dict['Default'][1] * (params['scale'] or 1)
    if bearing_dict[key][2] != '':
      font[g].right_side_bearing = bearing_dict[key][2] * (params['scale'] or 1)
    else:
      font[g].right_side_bearing = bearing_dict['Default'][2] * (params['scale'] or 1)
...
#save_font(font)
with open("test-%s.html" % font.fontname, 'w') as f:
  f.write(test_font_html('sachacHand-New.woff'))
#+end_src
  ...
  POS? equal 61 61
  POS? exclam 33 33
  POS? quotesingle 39 39
  POS? quoteright -1 8217
  POS? quotedbl 34 34
  POS? quotedblleft -1 8220
  POS? quotedblright -1 8221
  POS? at 64 64
  POS? slash 47 47
  POS? backslash 92 92
  POS? asciitilde 126 126
  POS? underscore 95 95
  POS? numbersign 35 35
  POS? dollar 36 36
  POS? percent 37 37
  POS? ampersand 38 38
  POS? parenleft 40 40
  POS? parenright 41 41
  POS? asterisk 42 42
  POS? plus 43 43
  POS? comma 44 44
  POS? period 46 46
  POS? less 60 60
  POS? greater 62 62
  POS? bracketleft 91 91
  POS? bracketright 93 93
  POS? asciicircum 94 94
  POS? grave 96 96
  POS? braceleft 123 123
  POS? bar 124 124
  POS? braceright 125 125
  POS? q 113 113
  H H
  ...
  with tag('html'):
    ...
    with tag('body'):
      ...
      with tag('div'):
        ...
        line('span', "Hello world", klass='normal')
      with tag('table'):
        with tag('tr'):
          line('td', test_glyphs(font))
