...
*** Code to make the template
 ...
 def draw_letter(column, row, letter, params):
   draw = params['draw']
   origin = (column * params['em_width'], row * (params['em'] + params['row_padding']))
 ...
 def make_template(params):
   img = Image.new('RGB', (params['columns'] * params['em_width'], params['rows'] * (params['em'] + params['row_padding'])), 'white')
...
*** Actually make the templates
 ...
 #+begin_src python :results file :eval yes
   <<params>>
   <<def_make_template>>
   ...
   #  'template': 'template-kg.png', 'ascent': 675, 'descent': 375, 'caps': 525, 'x_height': 300, 'row_padding': 15}) 
   make_template({**params, 'font_name': 'sachacHand.otf', 'template': 'template-sachacHand.png', 'row_padding': 100})
...
** Generate fonts
 ...
 font = load_font('sachacHandBold.sfd')
 save_font(font, {**params, "new_otf": "sachacHandBold.otf"})
 font = set_bearings(font, bearings)
 font.removeLookup('kern')
 save_font(font, {**params, "new_otf": "sachacHandBoldKerned.otf"})
 font = kern_classes(font, kerning_matrix)
 font = kern_by_char(font, kerning_matrix)
 #+end_src
...
** Test the font
 ...
 fonts = [['raw', 'sachacHandRaw.otf'],
          ...
          ['bold-kerned', 'sachacHandBoldKerned.woff']]
...
pkg_resources.get_distribution("fontforge").version
#+end_src
