...
#+begin_src python :eval yes :results output :session "out" :bearings=regular_bearings
  ...
  for sub in ['calt', 'ss01', 'ss02']:
    if sub in font.gsub_lookups: font.removeLookup(sub)

  def get_stylistic_set(font, suffix):
    return [g for g in font if suffix in g]
  ss01 = get_stylistic_set(font, 'ss01')
  ss02 = get_stylistic_set(font, 'ss02')
  default = [glyph_base_name(g) for g in ss01]
  font.addLookup('calt', 'gsub_contextchain', 0, [['calt', [['latn', ['dflt']]]]])
  for sub in ['ss01', 'ss02']:
    font.addLookup(sub, 'gsub_single', 0, [])
    font.addLookupSubtable(sub, sub + '-1')
  for g in ss01: get_glyph(font, glyph_base_name(g)).addPosSub('ss01-1', g)
  for g in ss02: get_glyph(font, glyph_base_name(g)).addPosSub('ss02-1', g)
  # font.addContextualSubtable('calt', 'calt-1', 'glyph', '''
  # a | a @<gsub>
  # b | b @<gsub>
  # ''')
  font.addContextualSubtable('calt', 'calt-1', 'class', '1 | 1 @<ss01>', bclasses=(None, default), mclasses=(None, default))
  font.addContextualSubtable('calt', 'calt-2', 'class', '2 | 1 @<ss02>', bclasses=(None, default, ss01), mclasses=(None, default, ss01))
  font.familyname = 'sachacHand'
